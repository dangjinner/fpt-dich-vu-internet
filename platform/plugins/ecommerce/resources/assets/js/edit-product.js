class EcommerceProduct {
    ...
    handleChangeSaleType() {
        ...
        _self.$body.on('click', '.turn-on-schedule', (event) => {
            ...
            $group.find('.turn-off-schedule').removeClass('d-none')
            $group.find('.detect-schedule').val(1)
            $group.find('.scheduled-time').removeClass('d-none')
        })
        ...
        _self.$body.on('click', '.turn-off-schedule', (event) => {
            ...
            let $group = $current.closest('.price-group')
            $current.addClass('d-none')
            $group.find('.turn-on-schedule').removeClass('d-none')
            $group.find('.detect-schedule').val(0)
            $group.find('.scheduled-time').addClass('d-none')
        })
    }
    ...
    handleStorehouse() {
        ...
        _self.$body.on('click', 'input.storehouse-management-status', (event) => {
            ...
            if ($(event.currentTarget).prop('checked') === true) {
                $storehouseInfo.removeClass('d-none')
                $('.stock-status-wrapper').addClass('d-none')
            } else {
                $storehouseInfo.addClass('d-none')
                $('.stock-status-wrapper').removeClass('d-none')
            }
        })
    }
                ...
                success: (res) => {
                    if (res.error) {
                        ...
                        $('#add-new-product-variation-modal .select-attributes').map((index, el) => {
                            const data = productAttributeSets.find((item) => item.id === $(el).data('id'))

                            if (data) {
                                Botble.select(el, {
                                    options: data.attributes.map((item) => {
                                        return {
                                            value: item.id,
                                            text: item.title,
                                        }
...
$(() => {
    ...
    $(document).on('click', '.btn-trigger-add-attribute', (event) => {
        ...
        $('.list-product-attribute-wrap').slideToggle()
        $('.list-product-attribute-values-wrap').slideToggle()
    ...
    $(document).on('click', '.btn-trigger-remove-product-image', (event) => {
        ...
        if ($('.list-gallery-media-images').find('.product-image-item-handler').length === 0) {
            $('.default-placeholder-product-image').removeClass('d-none')
        }
    })
    ...
    $(document).on('click', '[data-bb-toggle="product-search-advanced"]', (event) => {
        ...
        let $formBody = _self.closest('.box-search-advance').find('.card')
        $formBody.show()
